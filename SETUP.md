...
Для раздела **НОВЫЕ ОСТАТКИ** (срок доставки как у клиента по городам) добавь:

```
WB_SITE_COOKIE=...   # cookie с www.wildberries.ru (см. ниже)
NEW_STOCK_ARTICLES_JSON=[{"vendor_code":"МОЙ-АРТ","nm_id":123456789}]
```

Как взять `WB_SITE_COOKIE`:
1. Залогинься на [wildberries.ru](https://www.wildberries.ru) (покупатель).
2. Открой любую свою карточку товара.
3. F12 → Network → Clear → F5.
4. В Filter: `__internal/card` → клик по `detail?...` → Headers → Request Headers → **cookie** → Copy value.
5. Вставь в Railway. Cookie протухает — если матрица пустая/403, обнови cookie.

`NEW_STOCK_ARTICLES_JSON` — список **своих** артикулов продавца и nmID. Без него подтянется чужой дефолт из кода — замени обязательно.
...
4. **НОВЫЕ ОСТАТКИ** → «Обновить». Ячейка = остаток + **клиентский** срок витрины (>40ч жёлтый, >60ч красный). Подсказка при наведении может показать ближайший склад (как MKeeper) — это не то же самое, что видит клиент.
5. Финансы: PIN по умолчанию `1234` (файл `frontend/index.html`, константа `FINANCE_PIN`) — смени под себя и задеплой снова. Данные кредитов/себестоимости живут в твоём Supabase, в коде их нет.
6. Склейки и кредиты — заполняй уже свои, чужих данных в шаблоне нет.

## 6. Что НЕ копировать у друга

- Его `WB_TOKEN`, Supabase, Railway variables, `WB_SITE_COOKIE`
- Его Google Sheet и список артикулов
- Его домен / CRM
...
| НОВЫЕ ОСТАТКИ чужие артикулы | Задай свой `NEW_STOCK_ARTICLES_JSON` |
| НОВЫЕ ОСТАТКИ err / пусто | Обнови `WB_SITE_COOKIE`, проверь деплой |
